...
/**
 * Permits control of a status line. The default instance may correspond to the
 * NetBeans status line in the main window.
 *
 * @author Jesse Glick
 * @since 3.14
 */
public abstract class StatusDisplayer {
    ...
    /**
     * Default message 'importance' for error and warning messages on current
     * line.
     */
     ...
     * <p>
     * Modules should <strong>not</strong> need to call this method. If you
     * think you really do, please explain why on nbdev. The implementation of
     * the GUI component (if any) which displays the text naturally needs to
     * call it.
     *
     * @return some text
     */
    ...
    /**
     * Show text in the status line. Can be called at any time, but remember the
     * text may not be updated until the AWT event queue is ready for it - so if
     * you are hogging the event queue the text will not appear until you
     * release it (finish your work or display a modal dialog, for example).
     * <p class="nonnormative">Default implementation of status line in NetBeans
     * displays the text in status line and clears it after a while. Also there
     * is no guarantee how long the text will be displayed as it can be replaced
     * with new call to this method at any time.</p>
     * <p>
     * Note: The text may not show in the status line at all if some other text
     * with higher importance is currently showing in the status line as status
     * messages displayed this way have zero <code>importance</code>. The
     * message will show when higher-priority message has been cleared (either
    ...
    /**
     * <p>
     * Show text in the status line. <code>importance</code> argument indicates
     * that the text should stay in the status line until it is replaced with
     * new text by calling <code>setStatusText(String,int)</code> again with the
     * same or higher importance value.</p>
     * <p>
     * The text will be removed from status line when this method's return value
     * is garbage-collected or explicitly by calling
     * <code>Message.clear(int)</code>.
     *
     * @param text The text to be shown until some other text with the same or
     * higher importance is passed into the status line.
     * @param importance Positive integer defining the 'Importance' of the
     * message to be displayed, the higher number the higher importance.
    ...
    public static interface Message {
         ...
         * milliseconds. The StatusDisplayer fires a <code>ChangeEvent</code>
