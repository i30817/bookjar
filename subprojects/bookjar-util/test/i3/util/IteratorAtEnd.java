 ...
 * Also add small section for my LinkedSet.listiterator in test(Collection c) since it is not a list
 */
...
public class IteratorAtEnd {
    ...
    static void test(Collection c) {
        ...
        if (c instanceof LinkedSet){
            final LinkedSet list = (LinkedSet) c;
            try {
                final SetIterator it = list.iterator();
                it.next();
                ...
                pass();
                check(! list.head.after.key.equals(x));
            } catch (Throwable t) { unexpected(t); }

            try {
                //to get to the end index where the next would throw NoSuchElementException
                //i need to  do this since the list.head is not part of the set
                //my iterator function never allows getting a index > all the elems
                final SetIterator it = list.new LinkedKeyIterator(list.head);
                it.previous();
                ...
                pass();
                check(! list.head.before.key.equals(x));
            } catch (Throwable t) { unexpected(t); }
